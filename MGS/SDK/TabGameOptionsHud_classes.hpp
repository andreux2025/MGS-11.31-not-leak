#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: TabGameOptionsHud

#include "Basic.hpp"

#include "FortniteUI_classes.hpp"
#include "Engine_structs.hpp"


namespace SDK
{

// WidgetBlueprintGeneratedClass TabGameOptionsHud.TabGameOptionsHud_C
// 0x0010 (0x0340 - 0x0330)
class UTabGameOptionsHud_C final : public UFortGameOptions
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame;                                    // 0x0330(0x0008)(ZeroConstructor, Transient, DuplicateTransient)
	class UCommonTextBlock*                       TooltipDisplayReference;                           // 0x0338(0x0008)(Edit, BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, ExposeOnSpawn, HasGetValueTypeHash)

public:
	void UpdateOptionsTab();
	void HUD_Changed(int32 Selected_Index, const struct FGameplayTag& Gameplay_Tag);
	void Construct();
	void CenterOnTab();
	void BndEvt__HUDCommonListView_K2Node_ComponentBoundEvent_5_OnListViewItemScrolledIntoView__DelegateSignature(class UObject* Item, class UUserWidget* Widget);
	void BndEvt__HUDCommonListView_K2Node_ComponentBoundEvent_37_OnListViewItemWidgetCreated__DelegateSignature(class UUserWidget* Widget);
	void ExecuteUbergraph_TabGameOptionsHud(int32 EntryPoint);

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"TabGameOptionsHud_C">();
	}
	static class UTabGameOptionsHud_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UTabGameOptionsHud_C>();
	}
};
static_assert(alignof(UTabGameOptionsHud_C) == 0x000008, "Wrong alignment on UTabGameOptionsHud_C");
static_assert(sizeof(UTabGameOptionsHud_C) == 0x000340, "Wrong size on UTabGameOptionsHud_C");
static_assert(offsetof(UTabGameOptionsHud_C, UberGraphFrame) == 0x000330, "Member 'UTabGameOptionsHud_C::UberGraphFrame' has a wrong offset!");
static_assert(offsetof(UTabGameOptionsHud_C, TooltipDisplayReference) == 0x000338, "Member 'UTabGameOptionsHud_C::TooltipDisplayReference' has a wrong offset!");

}

